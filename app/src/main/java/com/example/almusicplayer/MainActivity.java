...
public class MainActivity extends AppCompatActivity {
    ...
    public ArrayList<File> readOnlyAudioSongs(File file){
        ...
        for (File individvalFile : allFiles){
            if (individvalFile.isDirectory() && !individvalFile.isHidden()){
                arrayList.addAll(readOnlyAudioSongs(individvalFile));

            }else {
                if (individvalFile.getName().endsWith(".mp3")|| individvalFile.getName().endsWith(".aac")||individvalFile.getName().endsWith(".wav")||individvalFile.getName().endsWith(".wma")){
                    arrayList.add(individvalFile);
                }
            }
        }
